...
# 解决最长递增子序列问题的算法最低要求O(n log n)的时间复杂度，这里n表示输入序列的规模。

# 动态规划：
# 对于每个数字，我们可以选择是否放入递增子序列中，
# 如果放入，那么我们就可以得到一个更长的递增子序列，
# 如果不放入，那么我们就可以得到一个更短的递增子序列。
# 因此，我们可以用一个数组来保存每个数字的最长递增子序列的长度，
# 当我们遍历到一个新的数字时，我们可以更新它的最长递增子序列长度，
# 上面一段是Github Copilot自动生成的。。。
...
def func2(l):
    ...
    但迭代到6的位置的时候，    dp = [0, 1, 3, 5, 6]，用6替换了7，
    而迭代到2的位置的时候，    dp = [0, 1, 2, 5, 6]，用2替换了3，
    因为此时，所有长度为2的LIS中，末尾的数 最小的为2
    这个算法很奇特，要理解，dp并不代表最长子序列本身，dp[i]的定义是注释开头那个，如果理解不了这一点，那肯定无法理解这个算法。
    """
