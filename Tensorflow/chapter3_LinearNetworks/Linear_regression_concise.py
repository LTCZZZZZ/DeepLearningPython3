...
# 如果X的行列互换（即转置）的话，那么计算应变为 X=tf.matmul(W, X) + b，矩阵W的每一行向量w表示一个神经元对前一层输入的参数，
# 这样的话每次计算需要把W拿过来左乘，即 Y = tf.matmul(W2, tf.matmul(W1, X) + b1) + b2，式子连写出来顺序看起来稍微有些反直觉
import numpy as np
...
# activation参数不设置时，直接输出原始值，即不采用激活函数
